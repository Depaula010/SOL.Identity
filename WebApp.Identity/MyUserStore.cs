...
namespace WebApp.Identity
{
    public class MyUserStore : IUserStore<MyUser>, IUserPasswordStore<MyUser>
    {
        ...
        public async Task<IdentityResult> CreateAsync(MyUser user, CancellationToken cancellationToken)
        {
            ...
            {
                await connection.ExecuteAsync("insert into Users([Id], [UserName],[NormalizedUserName],[PasswordHash]) Values (@id, @userName, @normalizedUserName, @passwordHash)",
                    new
                    {
                        id = user.Id,
        ...
        public async Task<IdentityResult> UpdateAsync(MyUser user, CancellationToken cancellationToken)
        {
            using (var connection = GetOpenConnection())
            {
                await connection.ExecuteAsync("update Users set [Id] = @id, [UserName] = @userName, [NormalizedUserName] = @normalizedUserName, [PasswordHash] = @passwordHash where [Id] = @id",
                    new
                    {
                        id = user.Id,
                        userName = user.UserName,
        ...
        {
            var connection = new SqlConnection("Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=Identity_Curso;Data Source=DESKTOP-TUDNH08\\SQLEXPRESS");
            connection.Open();
            return connection;
        }
        #region FIND
